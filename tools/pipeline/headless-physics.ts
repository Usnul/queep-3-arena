...
export class HeadlessPhysics {
    ...
    /**
     * Contents queries stay on the ported clipmap.
     *
     * `CONTENTS_WATER`, `CONTENTS_LAVA` and friends are Q3 *semantics*, not
     * collision: a physics engine has no opinion on whether a volume is slime.
     * The brushes carrying them are not turned into bodies at all, so this is
     * not a fallback -- it is the only place that answer exists.
     */
    pointContents(p: ArrayLike<number>): number {
        return pointContents(this.cm, p[0]!, p[1]!, p[2]!);
    }
    ...
    /** `pm->trace` on meep physics. Q3 units in, Q3 units out. */
    trace(
        out: TraceResult,
        startQ3: ArrayLike<number>,
        endQ3: ArrayLike<number>,
        ...
        if (length < 1e-7) {
            ...
            if (
                shape_cast(this.system, ray, shape, NO_ROTATION, this.hit, undefined, false) &&
                this.hit.t <= 1e-6
            ) {
                out.allsolid = true;
                out.startsolid = true;
