...
### GAP-044: OPEN — a browser client standing still sinks a fifth of a unit into the floor the host has it standing on

- **Needed:** a client and a host that agree, byte for byte, about a player who is not moving. That is the cheapest case there is, and it is what the prediction short-circuit is measured on.
- **Observed, against `npm run host -- --map oa_dm1 --bots 2` over a real socket, standing still for five seconds:** every AUTH_STATE disagrees. After the `weaponTime` defect above it (D-178) was fixed, the remaining field is vertical: the host has the player at `z = -119.84` with `groundEntityNum = ENTITYNUM_WORLD` and `velocity[2] = 0`; the client has it at `z = -120.062` with `groundEntityNum = ENTITYNUM_NONE` and `velocity[2] = -13.33`, which is exactly one frame of Q3 gravity. The horizontal position, the view angles, the bob cycle, the ground normal, `pmFlags` and `viewheight` are all identical.
- **What the client's own physics world says about the two positions.** Traced 0.25 units down with the player box: from **the host's** `z` it finds the floor at `fraction 0.14`, `startsolid false`. From **its own** `z`, 0.22 units lower, it is `startsolid` and `allsolid` with `fraction 0`. So the client is *inside* the surface, `PM_GroundTrace` correctly reports no ground, gravity is applied for ever, and the solver's sweep holds the position steady — a stuck state that looks completely normal on screen and disagrees with the host on every frame.
- **Not established, and stated as unknown rather than guessed:** whether this is `src/app/main.ts`'s branch or an artifact of the measurement. The measurement drove `NetClient` on a `PhysicsWorld` built by hand in the page, because **the preview browser cannot start the renderer** (`requestAdapter()` returns null, so `EngineHarness` throws before `main()` reaches this branch at all). That hand-built world is missing two things `main.ts` gives its own: the kinematic bodies `MoversView` creates for brush entities, and the static brush models `movers.statics` adds through `addStaticModel` before `optimize()`. If the surface at `(272, 1440)` is one of those, the client is standing on geometry the probe never built and the browser proper would not reproduce it.
- **Severity: unknown, and potentially major.** If it is real it costs the whole short-circuit — the client rewinds and replays its lead every tick, which is roughly ten movement steps a frame — while looking perfectly correct to a player. If it is the probe, it costs nothing. The measurement that separates the two is the same one either way: one browser with a GPU, `?join=`, and `window.queep.net.reconcileCount` over ten seconds of standing still.
- **Evidence:** `src/app/main.ts` (the `?join=` branch), `src/app/netSystems.ts`, `src/client/net/NetClient.ts` (`ringHash`, `shortCircuitDisagreed`), D-178 for the field-by-field method and for the defect this one was hiding behind.
