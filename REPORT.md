...
### GAP-038: nothing is replicated into existence, so every networked thing has to exist before the first client connects
...
- **Suggested fix:** a spawn action alongside `ReplaceComponentAction`, carrying `network_id` plus the initial component set, applied by the same `#apply_initial_sync` walk that already exists and already does exactly this work for the join. Failing that, an `onUnknownEntity` signal on the replicator would let an application supply a factory instead of losing the packet. Failing *that*, a sentence in `NetworkSession`'s docblock — "entities must exist on every peer before they can be replicated to; there is no spawn replication" — would have saved a day, because the join path working perfectly is what makes the absence invisible.
- **Evidence:** `node_modules/@woosh/meep-engine/src/engine/network/NetworkSession.js` (`#make_replace_component_action_class`, `apply`), `orchestrator/NetworkPeer.js` (`NetworkPacketType`), `src/server/Host.ts` (`buildPools`), `src/net/protocol.ts` (`MAX_MISSILES`)
...
- **Evidence:** `src/client/net/NetClient.ts` (`buildPools`, the parking comment), `src/server/Host.ts:460` (the host's own rule and the failure it was written for), `test/net-loopback.test.ts` ("the bodies of slots nobody is playing"), D-179 for the two measurement traps that hid it.
