...
| | |
|---|---|
| meep | `@woosh/meep-engine@3.16.0` (peer dependency, never vendored). Findings recorded against 3.0.2 and 3.2.0 are dated as such; BUG-7 was fixed in 3.2.0. The gap register was last re-verified **in full** against 3.14.3 — see the status table at the top of section 3 for what closed. Since then 3.15.0 closed GAP-045 (D-193) and 3.16.0 moved the engine onto `Transform64`, opening GAP-049 and GAP-050 (D-206); the rest of the register has not been re-walked against either. |
| Node | v24.15.0 |
