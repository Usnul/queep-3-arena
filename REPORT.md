...
### GAP-047: OPEN — 3.14.6's own delivery counter says frames are still skipped unapplied on the default configuration
...
- **On the default, it is not zero.** Measured with `test/net-latency.test.ts`, 20 s, one client firing ten shots a second at 30 Hz, `max_packets_per_tick` untouched:

| link | `skipped_unapplied` | events delivered |
|---|---:|---:|
| 40 ms, 8 ms jitter, 1% loss | **8** | 302 / 302 |
| 80 ms, 20 ms jitter, 2% loss | **27** | 297 / 300 |
| 150 ms, 40 ms jitter, 5% loss | **80** | 288 / 300 |

- **The event counts are nearly perfect anyway, and that is the point.** Most frames carry no event, so most skipped frames cost nothing visible — which is exactly why this took a purpose-built cross-peer measurement to find the first time, and exactly why the counter is the right thing to watch rather than the events. The two columns disagree about how healthy this is, and the counter is the more honest of them.
- **Not established:** whether these are frames that genuinely never ran, or whether the ring-based `applied` test in `Replicator` can report a false positive when a frame is older than the window it indexes (its own comment says a frame older than the window "answers no, which is right: it is beyond recovery either way" — that is right for recovery and may not be right for *counting*). A reader of this entry should treat the number as a signal to investigate, not as a proven loss.
