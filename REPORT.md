...
- **Workaround:** reach past the component. `Scene.lights.elements` is public, so a pass over the collection after the entities are built can write `radius` on Shade's own lights -- 83 lines of it, including the position match back to the source data, in `src/client/map/lightVolume.ts`. It works, and it is a workaround in the strict sense: the association between an ECS light and Shade's is private to `LightSystem3`, so the pass has to rebuild it from position, and it has to invalidate the collection by hand afterwards because a light changed in place does not. **And it does not generalise.** The second consumer to want the field was the per-weapon muzzle flash (D-115), where a shotgun's blast is a hand-sized ball of fire and a railgun's is a slit; a pass that rebuilds the association from position is affordable once at load and not for a light that is created per shot and moves every frame, so that table ships without the column it most wanted.
...
### GAP-038: nothing is replicated into existence — an entity can be *introduced* by a pushed snapshot, and never removed

- **Needed:** a rocket. A player fires, an entity appears on the host, and every client sees it appear. This is the most ordinary thing a networked shooter does.
- **meep offers:** a complete replication stack — `NetworkIdentity` marks an entity, `NetworkSystem` allocates it a peer-shared `network_id`, INITIAL_SYNC ships the whole world to a joining peer, and per-frame `ReplaceComponent` actions carry mutations. The join half is genuinely complete: `NetworkSession.#apply_initial_sync` creates a local entity for any `network_id` it does not know, so a client that starts empty ends up holding the host's whole world.
- **The gap is everything *after* that.** `ReplaceComponentAction.apply` opens with `const local = executor.slot_table.entity_for(this.network_id); if (local < 0) return;` — a silent return for an entity the receiver has never heard of. `STATE_BURST` (the recovery path) walks entities that already exist. There is no CREATE or DESTROY packet type in `NetworkPacketType`, and no signal a receiver could hang a factory off. So a `NetworkIdentity` attached on the host after a client has joined is, to that client, an entity that does not exist and never will — and the host is told nothing, because a silent return is not an error.
- **Re-measured on 3.15.0, and the shape is narrower than this entry said.** The register read as though every networked thing must pre-exist. Two of the three cases were tested rather than assumed (D-194):

| the host does this, to a client already connected | result |
|---|---|
| creates an entity and publishes it for ninety frames | **never arrives** — `entity_for` stays -1, silently, exactly as below |
...
| destroys the entity and pushes a fresh INITIAL_SYNC | **the client keeps it** |
  ...
  So there *is* a way to introduce an entity to a connected peer: `NetworkPeer.send_initial_sync` is public, and `NetworkSession.#apply_initial_sync` walks the wire format by hand rather than using `snapshotter_apply` **specifically so that a slot which is already allocated does not throw** — re-sync is a case the engine anticipated and the docblock says so. What has no answer at all is removal: a snapshot only creates entities the receiver does not know, so an entity the host has destroyed lives on every client for ever. Two costs come with the push: it is a whole-world snapshot per join per peer, and it overwrites the peer's session token (`#peer_session_tokens` is `#`-private), so a later resume degrades to a fresh sync.
- **What this port does now (D-194).** Players are created when somebody arrives and destroyed when they leave: a pushed INITIAL_SYNC introduces them, and an application-level `PlayerLeft` action takes them away, because the host cannot. Missiles and items are still pools, and correctly so — they are a *fixed* population, the same count on both peers in the same order, which is what makes their network ids line up by construction. The workaround below therefore survives for the half of the problem it was actually needed for.
- **Workaround (as filed, and still what missiles and items do):** pools. Sixteen player slots, sixty-four missile slots, one entity per map item, one per mover and one match entity, all built before `session.start()` returns, all present in the INITIAL_SYNC every joining client gets. A rocket is a pool slot whose `active` byte goes to 1, not an entity that appears; a `generation` counter rides beside it so a client blending between two ticks can tell a reused slot from a rocket that teleported. The pool is sized by the worst case — a plasma gun at ten rounds a second with a ten-second fuse tops out near a hundred in flight if nothing it fires ever hits anything, and 64 is the judgement made against that.
- **Severity:** major, and structural rather than annoying. It decides the shape of the game code: every system that would naturally spawn has to be rewritten around a fixed table, the table's size becomes a balance decision, and a pool that runs out silently drops the shot. It also costs bandwidth — sixteen player slots are published whether sixteen people are playing or not — and it costs a subtle correctness property, because a pool slot has a history and a fresh entity does not.
- **Suggested fix, and the second half now matters more than the first:** a spawn action alongside `ReplaceComponentAction`, carrying `network_id` plus the initial component set, applied by the same `#apply_initial_sync` walk that already exists and already does exactly this work for the join — **and a despawn to go with it**, because a consumer can already fake the spawn with a pushed snapshot and cannot fake the despawn with anything. Failing that, an `onUnknownEntity` signal on the replicator would let an application supply a factory instead of losing the packet. Failing *that*, a sentence in `NetworkSession`'s docblock — "entities must exist on every peer before they can be replicated to; there is no spawn replication" — would have saved a day, because the join path working perfectly is what makes the absence invisible.
- **Evidence:** `node_modules/@woosh/meep-engine/src/engine/network/NetworkSession.js` (`#make_replace_component_action_class`, `apply`, `#apply_initial_sync`'s re-sync note), `orchestrator/NetworkPeer.js` (`NetworkPacketType`'s thirteen entries, `send_initial_sync`), `src/server/Host.ts` (`buildPools`, `addPlayer`, `removePlayer`, `pushSnapshot`), `src/client/net/NetClient.ts` (`watchForPlayers`, `reapPlayer`), `src/net/actions.ts` (`PlayerLeft`), `src/net/protocol.ts` (`MAX_MISSILES`), D-194

### GAP-039: `onLocalSim` re-runs under rollback and its docblock asks for idempotence a game cannot give

- **Needed:** the server's per-frame game logic — bots think, projectiles age, items respawn, players die and come back — run once per frame, in the frame the netcode is currently simulating.
- **meep offers:** exactly the right seam. `ServerAuthoritativeServer.onLocalSim` fires inside `#replay_frame`, after every input for that frame has been applied and *before* the action-log frame is closed, which is the only window in which a server-originated mutation can be recorded. `SimActionExecutor.execute` writes into `ActionLog.current_buffer()` unconditionally and that throws `no frame is open` outside it, so this is not merely the convenient place, it is the only legal one.
- **The gap is what the docblock then asks for:** "the local sim should be idempotent under repeated application (because it may run many times under rollback)". A game's world step is the one thing that cannot be. A bot's behaviour tree advances, a missile moves, an item's respawn timer fires, a player's two-second death clock ticks. Run it twice for the same frame and the match is a different match; run it four times, as a four-frame rollback does, and it is four different matches. There is no "replay" flag on the signal, no way to ask the orchestrator whether this frame is being re-simulated, and `current_sim_frame` is not published until after the whole replay loop finishes — so the handler cannot even tell.
- **Workaround:** the application keeps its own high-water mark. `Host.onLocalSim` returns immediately unless `frame > simulatedThrough`, so the world step runs only for the newest frame in a replay window; the older frames get their state back from the historical `ReplaceComponent` records the log already holds, which is the mechanism the rewind is built on anyway. The cost is real and worth naming: a client input that arrives late is applied to the frame it belongs to, but resolves against a world the host has since moved on — a bot standing where it now is, not where it was. For a 60 Hz host with a four-frame input buffer that is at most 67 ms of staleness on the *non-player* half of the world, and Q3 vanilla has no better answer.
- **Severity:** major. The docblock's advice is not achievable by the class of application the orchestrator is for, and following it literally is impossible rather than merely expensive — so every consumer will invent the same high-water mark, and the ones that do not will have a match that quietly diverges under load, which is the failure mode with the longest debugging tail there is.
- **Suggested fix:** pass a second argument — `onLocalSim.send2(frame, is_replay)` — which the orchestrator already knows (`replay_start <= committed_top` decides it) and which costs one boolean. Or expose `sim_frame` before the replay loop so a handler can compare. Either turns a paragraph of advice into a fact the handler can act on.
- **Evidence:** `node_modules/@woosh/meep-engine/src/engine/network/orchestrator/ServerAuthoritativeServer.js:232-246` (the docblock), `:350-392` (`tick`), `:410-470` (`#replay_frame`), `src/server/Host.ts` (`onLocalSim`, `simulatedThrough`)

### GAP-040: ownership is decided at attach time and never travels, so a client owns the whole world until it says otherwise

- **Needed:** a client that predicts its own player and interpolates everybody else's. That split is the entire client-side model and the engine implements both halves.
- **meep offers:** `NetworkIdentity.owner_peer_id`, and a facade that reads it at exactly the right moments. `#on_identity_attached` puts every non-owned entity into `#remote_entities` — the set that gets blended at render time and normalized before simulation — and `OwnerAwareScope` uses it to keep a peer's own entities out of the stream sent to that peer.
- **The gap is that ownership is local knowledge presented as shared knowledge.** `#on_identity_attached` fills a negative `owner_peer_id` with *the local peer's own id*, which is right for a host spawning its own world and wrong for a client building the same pool: every entity a client creates is, by that default, owned by the client. INITIAL_SYNC does not correct it, and cannot — `NetworkIdentitySerializationAdapter`'s own docblock says the component "doesn't go on the wire" because it is a save-game adapter — so nothing on the join path ever tells a client which entities are the host's. The membership decision is also taken **once**, at attach, so a later write to `owner_peer_id` fixes the scope filter and does not fix `#remote_entities`.
- **What it costs, measured.** Two failures, neither of which names its cause. Nothing is interpolated, because `#remote_entities` is empty. And the client's outbound stream carries everything: the replicator logs every action it *applies* into the local action log — correct, the client's own rewind needs those records — and `flush_outbound` then packs that same log for every peer, so with the default `AlwaysRelevantScope` a client returns the host's entire state stream to the host a few frames stale, and the host applies its own state to itself. Observed: the host's player slot teleported 64 units downward on the first echoed frame and then fell out of the level, with the client's numbers arriving as the host's authority. It reads as a physics bug for as long as you let it.
- **Workaround:** set `owner_peer_id` explicitly *before* attaching each `NetworkIdentity` — the host for everything except this client's own slot — and set `OwnerAwareScope` on the client's replicator after `start()`, which the engine documents only for the server side and which is exactly as necessary on the client.
- **Severity:** major. Both symptoms are silent, neither points at ownership, and the defaulting rule is documented as a convenience ("`-1` means the local peer") rather than as the decision it actually is.
- **3.14.6 makes the symptom visible, which is half the problem.** `NetworkSession.remote_entity_count` exposes the size of `#remote_entities`, and its docblock names this failure exactly: zero on a connected client that has received a snapshot "means nothing will be interpolated and the client is about to send the host its own state back". That is the observability this entry asked for and did not expect to get. The defaulting rule itself is unchanged, so the workaround stays -- but a client that gets it wrong can now be *caught* rather than deduced from a host teleporting 64 units downward.
- **Suggested fix:** carry `owner_peer_id` in INITIAL_SYNC — the snapshot already walks every replicated component for every entity, and this is two bytes per entity on a packet that is sent once per join. Failing that, say in `replicate`'s or `NetworkSession`'s docblock that a client must assign ownership itself before attaching, and that `OwnerAwareScope` belongs on both ends.
- **Evidence:** `node_modules/@woosh/meep-engine/src/engine/network/NetworkSession.js` (`#on_identity_attached`, `#apply_initial_sync`), `src/engine/network/ecs/serialization/NetworkIdentitySerializationAdapter.js` (the "not for the replication stream" note), `src/engine/network/replication/ScopeFilter.js` (`OwnerAwareScope`), `src/client/net/NetClient.ts` (`identityOwnedBy`, `wireScope`)

### GAP-049: the physics queries read a pose by index and their published types say `{x, y, z, w}`, so a wrong call compiles and returns "nothing hit"

- **Needed:** a box sweep and an overlap against the level, which is every trace this port makes. `PhysicsTrace` is the whole collision backend behind `pmove`, and `DamageQueries.clientsInRadius` is `G_RadiusDamage`.
- **What changed in 3.16.0.** `shape_cast`, `overlap_shape` and `KinematicMover.move` moved to reading their pose arguments *by index* -- `rotation[0]`, `position[1]`, `v3_write(position, x, y, z)` -- as part of the `Transform64` migration, where a transform's translation and rotation are `Float64Array` views rather than objects. That is a reasonable change and the right direction.
- **The JSDoc and the generated `.d.ts` did not move with the code.** `shape_cast.js` still says `@param {{x:number,y:number,z:number,w:number}} rotation`, and `shape_cast.d.ts` still declares that object type; `overlap_shape` says the same for both `position` and `rotation`. `KinematicMover.move`'s declaration still says `position: Vector3` while its body indexes and `v3_write`s through it -- there the JSDoc for `rotation` alone was updated, to `ArrayLike<number>`, so the two halves of one signature now disagree with each other.
- **The failure mode is the bad one.** An object literal satisfies the declared type, so it compiles; at runtime `rotation[0]` is `undefined`, the arithmetic goes to NaN, and a NaN comparison is false everywhere -- so the sweep reports **no hit** rather than throwing. A collision backend that silently stops colliding. This port passed a `{ x: 0, y: 0, z: 0, w: 1 }` constant named `NO_ROTATION` in four files; `test/surface-metadata.test.ts` reported it as "no sweep landed, so this measured nothing", and `test/pmove.diff` and the player-controller suite as players falling through the world.
- **`Vector3` and `Quaternion` would satisfy both conventions and cannot be used to say so.** Both extend `Float64Array` and alias `v[0]` onto `v.x`, so either answers a name-based read and an index-based one -- exactly the type to pass while the two disagree. But both shadow `length` with the magnitude method (`length(): number`), so neither is assignable to the `ArrayLike<number>` that the *updated* signatures, such as `PhysicsSystem.overlap`, now require. The engine's own vector types do not satisfy the engine's own new parameter type.
- **Workaround:** plain `Float64Array` for every pose handed to a query -- `new Float64Array([0, 0, 0, 1])` for the identity rotation, a 3-element scratch for a query centre -- with the reasoning written at each declaration. `PhysicsTrace` additionally retypes the two free functions locally (`shape_cast_indexed`, `overlap_shape_indexed`), restating only the pose parameters, because the published declarations reject the correct argument. `PhysicsSystem.setPose` is the exception and is left alone: it still reads `.x`/`.y`/`.z`/`.w` by name, so `CharacterBody` passes a `Vector3` and a `Quaternion` there.
- **Severity: high, because it is silent and it is in the collision path.** Nothing throws, nothing warns, and the types actively certify the wrong call. The only reason this port found it in an afternoon is that it has a differential harness and 1,100 tests; an application with neither would see players walking through walls and have no thread to pull.
- **Suggested fix:** regenerate the declarations from bodies that were changed, and correct the three JSDoc blocks -- `ArrayLike<number>` is the honest type and is what `PhysicsSystem.overlap` already says. Separately, `Vector3.length` and `Quaternion.length` shadowing the typed-array property is worth a look on its own: it is what stops the engine's own vectors from being passed to the engine's own `ArrayLike` parameters, and `lengthSqr`/`magnitude` naming would free the slot.
- **Evidence:** `node_modules/@woosh/meep-engine/src/engine/physics/queries/shape_cast.{js,d.ts}` (`rotation` at the `@param` and at the declaration, against `rotation[0]` in the body), `.../queries/overlap_shape.{js,d.ts}`, `.../control/first-person/collision/KinematicMover.{js,d.ts}` (`v3_write`, and the `position: Vector3` declaration), `src/client/PhysicsTrace.ts` (`NO_ROTATION`, `shape_cast_indexed`), `src/client/DamageQueries.ts`, `src/client/MeepMove.ts`, `test/surface-metadata.test.ts`, D-206

### GAP-050: the interpolation write-back blends into the component and announces nothing, so no blended pose reaches the screen

- **Needed:** a door, a bot and a missile drawn between fixed steps rather than on them. The simulation runs at 60 Hz and the display does not; `InterpolationSystem` is meep's answer and this port is built on it (D-081, D-200, and the whole of `src/client/interpolation.ts`).
- **meep offers exactly this, and its docblock still says so.** `InterpolationSystem`'s own words: "Because it writes the live component, all Transform-linked machinery (culling, particles, trails, attachments) renders" at the blended pose. Under `Transform` that followed from the component's design -- position, rotation and scale were live objects carrying signals, and every consumer hung off them.
- **3.16.0 removes the signals and does not replace them here.** `TransformPoseSerializationAdapter.deserialize` -- the write-back `InterpolationSystem.update` runs once per rendered frame per interpolated entity -- calls `setTranslation` and `setRotation` and returns. It does not call `updateMatrix()`, so the blended *rotation* never reaches the matrix that `TransformAttachmentSystem` composes through; and it does not announce the write, so `ShadedGeometrySystem`, `MeshSystem`, `LightSystem`, `ParticleEmitterSystem` and the attachment system -- all of which now place on that announcement rather than on a poll -- never learn the pose moved. **Still open at 3.17.0**, which renamed the announcement to `TRANSFORM64_EVENT_CHANGE` (D-207) and left this write-back untouched: `InterpolationSystem.js`, `pose_interpoland.js` and `TransformPoseSerializationAdapter` are byte-identical across the two releases.
- **So the blend is computed, written, and discarded.** What gets drawn is the pose current at the last *announced* write, which for a mover is `MoversView.update` on the fixed step and for a missile is `PhysicsSystem`'s own announcement at the end of its step. Every interpolated thing in the port silently reverts to stepping at the simulation rate. This is a regression in a documented property and it is invisible to any headless assertion that reads the component, because the component is correct -- only nothing is listening.
- **Workaround:** `AnnouncingInterpolationSystem` in `src/app/systems.ts`, which is the two calls the write-back owes, applied after `super.update` to the entity set `InterpolationSystem` already maintains in its own `link`/`unlink`. **A subclass rather than a second system, deliberately:** meep orders systems by declared component access rather than by registration, so an announcer registered alongside could not be made to run strictly after the blend; overriding `update` settles it by construction and takes the scheduler out of it. Announcing per entity rather than per interpoland is the coarse-by-design trade `TRANSFORM64_EVENT_CHANGE` documents.
- **Severity: moderate, and entirely presentational.** Nothing diverges and no state is wrong -- the simulation never reads the blend, and `PhysicsSystem.__interp_restore` puts the authoritative pose back before each step regardless. What is lost is the smoothing, which is the whole reason the subsystem exists, and it is lost without a counter or a warning.
- **Suggested fix:** `deserialize` cannot own this -- it is a serialization adapter and has neither the entity id nor the dataset -- so it belongs in `InterpolationSystem.update`, which has both: after the write-back for an entity, `updateMatrix()` if the interpoland touched rotation or scale, then `t64_announce_change`. The engine already does exactly this in `PhysicsSystem` for its awake set, so the shape is established. Until then the docblock quoted above is a promise the code does not keep, and correcting it would be worth doing even ahead of the fix.
- **Evidence:** `node_modules/@woosh/meep-engine/src/engine/interpolation/TransformPoseSerializationAdapter.js` (`deserialize`), `.../InterpolationSystem.js` (the docblock, and `update`'s call into the adapter at the end of the interpoland loop), `.../graphics3/ShadedGeometrySystem.js` (`on_transform_changed`), `src/app/systems.ts` (`AnnouncingInterpolationSystem`), `test/interpolation.test.ts` ("announces the write, so a placement listener wakes on it" and "refreshes the matrix, which is what an attachment composes through"), `test/missile-view.test.ts` (the attachment case that found it), D-206

### GAP-048: a replay re-applies event actions, so a transient a client cannot predict is presented twice

- **Needed:** a muzzle flash, an explosion, a teleport sound to happen once. A `SimAction` that declares no affected components is this port's wire form of an *event* -- `EffectEvent`, `HitEvent`, `PickupEvent` -- and its whole effect is a side effect outside the replicated world: a particle system, a light, a positional sound, a view kick.
- **meep offers exactly the property this was built on, and offered it until 3.15.0.** `SimAction`'s contract is that an action with no affected components is one the `Replicator` always sends and the `RewindEngine` never touches. `src/net/actions.ts` wrote that down in step 1 and added the consequence: such an action "cannot be un-fired by a rollback, and cannot be fired twice by a retransmission".
- **3.15.0 makes the second half false, as a side effect of fixing something worse.** GAP-045's fix is `ServerAuthoritativeClient.#execute_harvested`: before replaying a rewound window, the client copies out every record in it whose `sender_id` is a connected peer and re-executes them, because an arrived record is the only description of the state it carries and dropping it loses that state for every entity the AUTH_STATE does not name. That reasoning is correct and the fix was worth having (D-193). It is also indiscriminate: the harvest tests provenance and never asks whether the record *carries* state, so an action whose `apply` only draws and plays runs a second time -- and a third, for a two-frame window.
- **It is worst exactly where it is most visible.** A client duplicates a transient only when it reconciles across the frame the transient arrived in, and the transients a client *cannot predict* are the ones that force that reconciliation: a teleport moves the player 620 units in a frame the client did not predict, so the AUTH_STATE that carries it is also the one that rewinds past its own `EffectEvent`. Measured on `oa_dm1`: one host-side teleporter crossing, one `EffectKind.Teleport` raised, **two** arrivals at the client, two `world/telein`s. A death, a pad and a pickup are the same shape. In an ordinary firefight it is a smaller tax on a bigger number -- 53 re-applications over 1,200 frames and 217 reconciliations, against 543 genuine events.
- **Workaround, and it costs nothing:** `NetClient` sets a `replaying` flag on `onBeforeReconcile` and clears it on `onReconcileComplete` and `onReconcileAbandoned` -- the two ways a reconciliation can end -- and the `effect`, `hit` and `pickup` hooks return early while it is set. It cannot drop a first delivery, because `#execute_harvested` only re-runs records already in the action log for the rewound frames, so everything it produces has been through the hook once. `test/net-effects.test.ts` holds the number the gate exists for: over the measured window the client presents 543 transients against 543 the host raised, exactly. `playerLeft` is deliberately **not** gated -- it is a state change rather than a presentation, so a replay is entitled to re-run it, and re-running it is free.
- **The alternative this did not need:** a sequence number on each event action and a seen-set on the client. It is robust against a genuine duplicate as well, and it costs one to three bytes on every event on a wire that already sends 543 of them a run. The flag is free and covers the case that actually exists.
- **Severity: moderate, and sharp-edged.** Nothing diverges -- the simulation is untouched, because these actions mutate nothing -- so the symptom is entirely presentational and entirely silent to every test that counts state: a doubled sound, a doubled explosion, a doubled view kick on a rocket that hit you once. It is also a *regression in a documented property*, which is the part worth reporting: an application that read `SimAction`'s contract and built its event layer on it, as this one did, gets the duplication with no warning and no counter.
- **Suggested fix:** skip a record in `#harvest_arrived` whose action declares no affected components, which is the engine's own statement of "this record carries no state". `SimAction.affected_components` is already called by the executor on the way in, so the answer is available; a flag on the class (`static is_event = true`) would be cheaper still and would say the same thing out loud. Either restores the property `SimAction`'s docblock describes without giving up GAP-045's fix, which is about records that *do* carry state.
- **Evidence:** `node_modules/@woosh/meep-engine/src/engine/network/orchestrator/ServerAuthoritativeClient.js` (`#harvest_arrived`, `#execute_harvested`, and the provenance note above them), `src/net/actions.ts` (the module docblock, and what it used to claim), `src/client/net/NetClient.ts` (`replaying`, `replayedTransients`), `test/net-effects.test.ts` ("presents every event that arrives, and drops none of them", and the teleport), D-197

### GAP-041 (FIXED in D-202; narrowed in D-191): the host had no *solid* movers, because the headless physics harness built model 0 only -- and its triggers never needed one

- **FIXED in D-202, and the entry below is kept because how it stayed open is the useful part.** `HeadlessPhysics.addMover` builds a brush entity's hulls as `KinematicVelocity` bodies -- the same hulls through the same `hullShape` as `PhysicsWorld.addMover` -- and `Host.worldStep` writes each mover's origin into them every frame. **The blocker named below was retired by D-191 and nobody noticed for two entries:** the harness built model 0 alone because models 1..n "are brush entities whose positions are owned by a mover simulation the harness has never had", and D-191 gave the host one. It is opt-in, because the *divergence* harness still has no movers and brush entities at their authored positions there is the D-036 failure this entry's own workaround section describes. What made it urgent was D-200 making the movers *visible* on a client: a door that stops nobody is invisible until it opens on your screen, and then a platform extends over `oa_dm1`'s lava pit, invites you on, and the authority drops you through it.
- **Needed:** doors, plats, buttons and triggers on the dedicated host, so a networked match has the map the single-player game has.
- **Half of that is done, and this entry had the scope wrong.** A mover has to be **solid** -- a kinematic body the player's sweep hits and can stand on -- and a trigger does not: `MoverSystem.touch` is a box-overlap test against bounds the BSP submodel table already carries, and a `trigger_teleport` never moves and is never solid. So teleporters, jump pads and hurt volumes needed no physics world at all and had been reachable since step 3; they run on the host as of D-191, with `test/net-triggers.test.ts` measuring all eight of `am_thornish`'s pads launching at exactly the vector `AimAtTarget` solved for. What is still blocked is doors and plats, and the sentence below is about those.
- **meep offers:** `PhysicsWorld.addMover`, which this port already uses in the browser to make a brush entity a kinematic body the player can stand on and be carried by.
- **The gap is this repository's, not the engine's, and is recorded here because a reader of `Host.ts` will otherwise look for it.** `HeadlessPhysics` builds BSP model 0 and nothing else — deliberately, and for a good reason: models 1..n are brush entities whose positions are owned by a mover simulation the harness has never had, and including them at their authored positions once made the harness disagree with both the clipmap control and the browser (the D-036 failure). The host inherits that limitation, so `MoverSystem` is not wired and `NetMover` is a component with an adapter, a wire slot and no producer.
- **Workaround:** none, and the wire format is arranged so that none is needed later. `NetMover` and its interpolation adapter are registered in `registerProtocol` from the start, so the protocol version does not change on the day the host grows movers — only the pool fills.
- **Severity:** minor as a gap and **reduced** as a missing feature, because the jump pads were the load-bearing half and they work. What is left is a door that blocks nobody and a plat that carries nobody on the host, which is a fidelity loss rather than an unreachable map. Giving the host `PhysicsWorld.addMover` (or lifting the three body-building calls into a module both harnesses share) closes it, and `WorldEffects.applyTouch`'s deliberately absent carry step is the one line that changes with it --
  and it changed in D-205, which also found that a mover's `BrushHull` planes were left at the
  authored position while its body moved, so an opened door had its contacts ruled on against the
  closed one.
- **Evidence:** `tools/pipeline/headless-physics.ts` (the model-0 comment), `src/server/Host.ts` (`buildPools`, and the trigger pass in `worldStep`), `src/game/WorldEffects.ts` (`applyTouch`, and why it does not carry), `src/net/components.ts` (`NetMover`), `test/net-triggers.test.ts`, D-191

### GAP-046: FIXED — replicated missiles now have a picture, through a second pool of render entities

- **Needed:** a rocket a networked client can see. Before this, a client saw the muzzle flash and the explosion — both are `EffectEvent`s and both arrive — and nothing in between, so a rocket crossing a room was invisible and the blast appeared unprovoked.
- **Why it was not simply `MissileView` on the replicated pool.** `MissileView.spawn(projectileId, entity, weapon, velocity)` builds child entities carrying `TransformAttachment { parent: entity }`, so it needs a parent that exists in the **render** dataset and already has a `Transform` something else moves. In single-player that something is the physics body the missile flies as. A joined client has no such body — the position is replicated — and `NetClient`'s pool lives in its own `EntityManager`, carrying `NetworkIdentity` and the net components and nothing else.
- **Fixed** with one render entity per pool slot, holding a bare `Transform`, and `NetPresentationSystem` writing `sceneFromQ3(NetMissile.origin)` into it once per rendered frame. A model appears when `active` goes to one, goes away when it returns to zero, **and is replaced when `generation` changes underneath it** — which is the case the counter exists for, because the host reuses a slot the moment its missile dies.
- **Placed before it is spawned**, on the frame it appears. The plan's wording was to hide a reused slot for a frame; placing first is the same idea without the missing frame, since the attached children read the parent transform the moment they exist.
- **Measured**, 600 frames with three bots on `oa_dm1`: 2 rockets spawned, 2 despawned, none left hanging, 600 roll updates, and a worst single-frame move of **30 units** for a slot that kept the same missile — about two frames of a rocket's 900 units a second, which is the render delay moving rather than a slot changing hands unseen. `test/net-presentation.test.ts` holds all four.
- **One thing this had to route around.** `Arena.update` is where single-player gets `CG_Missile`'s roll, and it cannot be called on this branch, because the first thing it does is step the weapons and the weapons are the host's. The roll comes through `MissilePresenter.advance` instead.
- **Evidence:** `src/app/netSystems.ts` (`MissilePresenter`, `drawMissiles`), `src/app/main.ts` (the render-entity pool), `src/client/MissileView.ts` (`spawn`, and the parent it requires), D-181.

### GAP-047: OPEN, upstream — the delivery counter counts the receiver's own held slices, so it reads 214 where 10 frames were lost

- **Needed:** confidence that GAP-043's residual is gone. 3.14.5 sliced a tick's owed range across up to `max_packets_per_tick` packets and closed the event loss this port measured at 41%; what stayed open was that the skip is *silent*, so there was no way to tell a healthy duplicate from a lost frame except by counting events at both ends.
- **3.14.6 supplies exactly the instrument that was missing.** `NetworkSession.delivery_stats(peer_id)` returns `{skipped_unapplied, skipped_duplicate}` per peer: the first counts frames that reached the client and were dropped below the applied watermark **without ever running**, the second the action stream's own redundancy being correctly ignored. Its docblock says the first "should stay at zero" on the default and climbs under jitter only at `max_packets_per_tick: 1`. That is a genuinely excellent addition and it is what makes this entry possible at all.
- **On the default it is not zero, and it is also not the loss.** Measured with `test/net-delivery.test.ts`, 20 s, one client firing ten shots a second at 30 Hz, four bots, `max_packets_per_tick` untouched. The "lost" column is taken independently of the counter: `onFrameApplied` gives the frames the client actually ran, and each inbound packet's slice header gives the frames the host put on the wire, so a frame in the second set and not the first was **delivered and dropped**.

| link | delivered and never applied | `skipped_unapplied` | head slices | non-head | events |
|---|---:|---:|---:|---:|---:|
| 150 ms clean, lossless | **0** | **0** | 0 | 0 | 318 / 318 |
| 40 ms, 8 ms jitter, 1% loss | **0** | 10 | 0 | 10 | 315 / 315 |
| 80 ms, 20 ms jitter, 2% loss | **0** | 29 | 0 | 29 | 600 / 603 |
| 150 ms, 40 ms jitter, lossless | **10** | 214 | 11 | 203 | 770 / 788 |
| 150 ms, 40 ms jitter, 5% loss | **13** | 225 | 12 | 213 | 840 / 864 |

- **The cause, and it is one argument.** `#hold_slice` validates a slice it is about to keep by calling `#apply_groups(peer, buf, end, Infinity, ...)`. `Infinity` means "skip every frame", which is the right instruction for a validation walk — but the skip branch is where the delivery accounting lives, so **every frame of every held slice is booked `skipped_unapplied` on the way in**, and then applied when the gap before it fills. Reordering is what creates held slices, which is why the counter reads 0 at 150 ms *clean* and 214 at the same latency with 40 ms of jitter: identical delay, identical redundancy window, and the only difference is whether a slice can arrive early. Every packet that incremented the counter applied nothing during its own call — 152 of 152 at the worst link.
- **It is not a rate, either.** On one link, a 10 s run reports 185 and a 20 s run 214, while the honest count is **ten in both**. A number that grows with how long two peers talk is counting traffic.
- **The part that works is the part that matters.** Only a *head* slice can lose a frame for good: `#apply_held_before` runs first and raises the watermark, and nothing before a head is ever re-sent. Split by slice kind, the head component is 11, 12, 0, 0, 0 against an honest loss of 10, 13, 0, 0, 0 — **within one at every link measured**. So `skipped_unapplied` restricted to head slices is exactly the instrument GAP-043 needed; unrestricted, it is that number plus the receiver's own held slices.
- **The ring window is not implicated**, which was the other hypothesis this entry carried. The re-sends that reach the skip branch are a median of 6 frames behind the applied top and at most 13, against a 64-deep ring, so `applied[frame % 64]` cannot have been overwritten. The comment about a frame older than the window answering "no" is correct and is not what produces these numbers.
- **And there is a real residual, small and mostly a join.** Ten frames out of ~540 on a **lossless** 150 ms link with 40 ms of jitter — lossless being the column that settles it, because the link dropped nothing, so those ten were delivered and discarded by the receiver. **Nine of the ten fall inside the first six seconds after the join**, in the same window as the rewind burst and the coherence dip, while `TimeDilation` is still walking to its buffer depth. At 40 ms and 80 ms it is zero everywhere. So GAP-043's residual does survive on default settings, at 1.9% of frames on the worst link and none at the two a player would use — the events this port does not lose are a consequence of that, not luck.
- **Suggested fix:** do not run the delivery accounting on the validation walk. Either pass a sentinel `min_frame` that `#apply_groups` can distinguish from a real watermark, or lift the two `#delivery_state` calls out of `#apply_groups` into the paths that genuinely apply and genuinely skip. Either makes the field mean what its docblock says, and the head-slice figures above are what it should then read.
- **Severity: moderate.** Nothing in the game is wrong; the instrument added to make a silent failure visible is loud in the healthy case, which costs a consumer exactly the investigation this entry records. It is also the field a consumer would most reasonably fail a test on — this port nearly did.
- **Reported upstream** at https://claude.ai/code/artifact/48a396e5-fa3b-4f59-9919-5e144f66b7fc, framed the way the GAP-043 report was: a standalone script that fails, a control that passes, and three outcomes listed as equally useful -- including "it is intended and the docblock is what needs changing" and "the measurement is wrong", with the assumption to attack first named explicitly.
...
- **Needed:** a client and a host that agree, byte for byte, about a player who is not moving. The cheapest case there is, and what the prediction short-circuit is measured on.
- **Observed:** a browser client standing still short-circuited **0 of 3,808** AUTH_STATEs — measured by the user in a real browser, against `npm run host -- --map oa_dm1 --bots 2`. Every one disagreed, so the client rewound and replayed its whole lead sixty times a second while looking perfectly correct on screen.
- **The gap is this repository's, not the engine's.** `Host.buildPools` parks an unconnected slot's body a million units below the map and documents why; `NetClient.buildPools` built the same pool of sixteen bodies and parked none. The local player's sweep hit a box the host does not have and was depenetrated **30.16 units — one whole player box** — after which its position disagreed with the host's on every frame.
- **Fixed** in `NetClient.buildPools`: a slot whose replicated `connected` is zero is parked at the host's own spot and spacing. On `oa_dm1` with one idle client and two bots, 600 frames: **0/600 short-circuited and 30.160 units of drift before; 590/600 and 0.000 units after**. In the browser against the real host: **0 of 3,808 before, 323 of 329 after**, the remainder being the once-a-second health bleed (D-170).
- **Why nothing caught it:** every other client in the suite walks, and a walking player leaves the parked body behind within a second. `test/net-loopback.test.ts` now stands still, and asserts the drift is zero rather than small.
- **Ruled out along the way, so they are not re-suspected:** `HeadlessPhysics` and `PhysicsWorld` agree exactly — same fraction, startsolid and floor height at every spawn point over 35 probes — and `uint8_array_hash` is deterministic over identical bytes at these lengths (it does return values outside 32 bits, which is startling but consistent).
- **And the parking is gone rather than kept (D-194).** Players are now created when they arrive and destroyed when they leave, so there is no body for a slot nobody is playing to be depenetrated off -- the failure has no material rather than a defence. `test/net-loopback.test.ts`'s parking test asserts that the two ends agree about the population, which is the property the parking was protecting.
- **Evidence:** `src/client/net/NetClient.ts` (`watchForPlayers`; the parking comment is gone with the pool), `src/server/Host.ts:460` (the host's own rule and the failure it was written for), `test/net-loopback.test.ts` ("the bodies of slots nobody is playing"), D-179 for the two measurement traps that hid it.

### GAP-042: FIXED IN 3.14.6 — a joining client's frame counter was never aligned to the host's, and there was no supported way to align it
...
| | |
|---|---|
| meep | `@woosh/meep-engine@3.17.0` (peer dependency, never vendored). Findings recorded against 3.0.2 and 3.2.0 are dated as such; BUG-7 was fixed in 3.2.0. The gap register was last re-verified **in full** against 3.14.3 — see the status table at the top of section 3 for what closed. Since then 3.15.0 closed GAP-045 (D-193), 3.16.0 moved the engine onto `Transform64`, opening GAP-049 and GAP-050 (D-206), and 3.17.0 renamed the transform's change announcement (D-207), closing neither — the files behind both gaps are byte-identical across 3.16.0 and 3.17.0. The rest of the register has not been re-walked against any of the three. |
| Node | v24.15.0 |
...
| Test suite | 1,188 tests across 59 files (38 skipped); `npm run check` typechecks, verifies the trap matrix and balance tables against their sources, and runs all of them. Three files need artefacts a fresh clone does not have — `cm-trace.diff`, `pmove.diff` and `physics-divergence` want `oracle/build`, and the trap matrix wants `.refs/` — so a checkout reports those as failures until `npm run setup` and the oracle build have run |
