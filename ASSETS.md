...
| `assets/built/*/audio-probes.bin` | `bake-audio.ts` | one to four minutes per map, six maps; needs only `collision.bsp`, so it can be re-run without re-converting anything |
...
| `assets/built/<map>/audio-probes.bin` | `bake-audio.ts` | the map's acoustic probe field: a position and a per-band RT60 for each of a few hundred probes covering its air, measured offline by casting rays at the same brush solids the physics bodies are built from. What `ProbeReverbRenderer` reads at the listener, so a hall rings and a corridor does not. Separate from `convert-map.ts` because it is minutes per map and depends on nothing the converter produces except `collision.bsp` |
