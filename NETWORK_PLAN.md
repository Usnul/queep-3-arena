...
| `EffectEvent` | host → all | none | `kind u8` (muzzle, hitscanTrail, bulletImpact, explosion, death, **teleport, jumpPad**), `weapon u8`, `owner u8`, `origin f32×3`, `aux f32×3` (direction / normal / end / destination / launch vector), `radius f32` | mirrors `WeaponEvents`, plus the two `MoverEvents` that make a noise (D-197); the client's sink is `Arena` minus damage, through `RemoteEffects`. A **zero `aux`** on an explosion means there was no surface, which is `CG_MissileHitPlayer`: Q3 marks walls and never marks people |
...
Event actions have no affected components, so `Replicator` always sends them and `RewindEngine`
never touches them. On the host their `apply` is a no-op.

**"And a client applies each exactly once" is no longer true, and the plan said it for four steps.**
`#applied_through` still refuses a duplicate off the wire; what arrived instead is a second path.
meep 3.15.0's `ServerAuthoritativeClient.#execute_harvested` re-executes every record in a rewound
window that arrived from a peer -- GAP-045's fix, and correct for a record that carries state,
because the record is the only description of it. An event action carries none, and the harvest has
no way to ask. So a client re-fires every transient inside every window it rewinds, worst exactly
where it is most visible: the transients a client cannot predict are the ones that force the rewind.
`NetClient.replaying` gates the three presentation hooks for the length of a reconciliation. See
GAP-048 and D-197.
...
- Bandwidth with `BandwidthMeter` on both transports at 0 ms and at 80 ms RTT, 8 humans + 4 bots
  ...
  **This number is undated and unexplained and should not have been asserted against** (D-196):
  48 KB/s is 384 kbit/s, a modem-era figure, and a sixteen-player match measured at 245 KB/s down
  is 1.96 Mbit/s -- fine on 3G and nothing after it. REPORT §5 carries the measurement and the
  retraction; the budget is kept here as filed because the tests were written against it.
...
**Engine: meep 3.17.0.** What each upgrade moved, so the numbers below can be read against the
...
| 3.16.0 | **nothing on the wire, and one component swap under it.** The `Transform64` migration (D-206) reaches `engine/network` in exactly one file: `TransformReplicationAdapter` is keyed on `Transform64` instead of `Transform` and reads the components as scalars. The byte layout is unchanged -- 3 x Float32 position, a uint32 quaternion, an equality-encoded scale -- and the Float32 position is still deliberate, so **f64 components do not widen a packet**; `TransformInterpolationAdapter` is byte-identical, and so is `Replicator.js`, so GAP-047 is untouched again. What did change is outside the network layer and matters to a joined client anyway: nothing hears a transform write until the writer announces it, and meep's own interpolation write-back does not (GAP-050), which is why `AnnouncingInterpolationSystem` exists |
| 3.17.0 | **nothing on the wire, and nothing under it either.** Not one file under `engine/network` differs from 3.16.0, so the byte layout, `Replicator.js` and GAP-047 are all untouched for the third release running. The release renames the transform's change announcement from `EventType.ComponentChanged` to `TRANSFORM64_EVENT_CHANGE` and drops its payload (D-207), which reaches a joined client only through `AnnouncingInterpolationSystem` -- the same two calls as before, on the new channel. GAP-050 is still open, so the workaround stays |
...
| 6 — full match for every slot | host half **done** (D-180); remote characters and missiles **done** and measured (D-181); client weapon selection **done** (D-182); **scoreboard HUD and remote footsteps done** (D-190), which found `bobCycle` publishing zero for every bot since step 3; **teleporters, jump pads and hurt volumes done on the host** (D-191) -- and GAP-041 was never blocking them, because a trigger is not solid. **GAP-045 closed upstream in 3.15.0** (D-193), so the scoreboard is now exact rather than one slot short: `net-match` reports zero stale and asserts it. And **players are created when they arrive rather than pre-created sixteen at a time** (D-194) -- a leaver's entity is destroyed and reaped on every client through a `PlayerLeft` action, which is the half of GAP-038 the engine has no packet for. **And every transient is presented** (D-197), which was the largest remaining hole: the `effect` and `pickup` hooks counted and dropped their events, so a joined client had no muzzle flash, impact, trail, explosion, death effect or pickup feedback at all. `NetTransients` drives `Arena` and `AudioBank` from the replicated stream rather than reimplementing either, `EffectKind` gains `Teleport` and `JumpPad` so that D-191's two silent triggers make Q3's noise, and an explosion with no surface now says so on the wire instead of substituting straight up and marking the floor under a body. Drawing anything at all also found a clock nobody was running: `Effects.update` is inside `Arena.update`, which this branch cannot call, so every flash and scorch mark would have stayed in the dataset for the whole match -- `NetEffectSystem` is the one line, measured as 220 entities created and 220 retired. Measured at **543 arrived, 543 presented, 543 raised** over 1,200 frames -- an equality that costs a workaround, because **GAP-048** is open: meep 3.15.0's replay re-executes arrived records, an event action carries no state to restore, and a transient a client cannot predict is therefore presented twice unless it is gated (one teleport crossing, two `world/telein`s). **And four passes a joined client was not running at all** (D-200), which is the same mistake three previous entries found one at a time: each single-player system is a simulation pass with a presentation pass welded to it, and the `netClient === null` branch drops them whole. Doors, plats and buttons did nothing (`WorldEffects.applyPresentation` is the half that is not the host's); every pickup on the map was invisible, because `ItemsView.update` places and shows them and only `PickupSystem` called it; the player heard everybody's footsteps except their own (`BodySounds`, now shared); and the local hitscan trail hinged off the drawn barrel to a far end a round trip old, so it bent away from the direction of travel. D-200 also found the **jitter**: `updatePresentation` ran once per *engine* frame rather than once per session step, so the camera's two-entry eye-pose history straddled half a step or none and stuttered at 30 Hz -- which looked like a prediction fault and was a clock. **The local player's own muzzle flash is predicted** (D-201), on the frame the trigger is pulled and through the same `calcMuzzlePoint` the host uses, with the host's copy of the shot dropped rather than drawn a round trip later on top of it: 230 of the host's 230 flashes land at a muzzle the client had already computed, to 0.000 units. The trail and the impact stay events, which is where Q3 draws the same line. **GAP-041 is closed** (D-202): `HeadlessPhysics.addMover` builds a brush entity's hulls as kinematic bodies and `worldStep` writes each mover's origin into them, so the host's doors and plats stop somebody -- the blocker the entry named was retired by D-191 and went unnoticed for two entries, until D-200 made the movers visible and `oa_dm1`'s platform started dropping players into the lava. D-202 also finished two of its own predecessors' halves: the camera's **blend** now spans the session period as well as its poses (D-200 moved the poses and left the alpha on the engine's step, which made the judder worse), and the local tracer is **predicted** rather than drawn from the host, because a beam has two visible ends and no arrangement that keeps a stale far end can attach the near one to the gun. **What is left:** triggers unpredicted, so a pad costs one correction; and bots use no trigger on either path |
| 7 — latency, loss, bandwidth | **done**. Both REPORT §5 tables written (D-183) and re-taken at 30 Hz (D-184): downstream **45.8 KB/s** per client, inside the 48 KB/s target; host CPU **29–65 ms/s**; event delivery now exact at every link. Superlinear host cost unchanged. **A relevance filter was built against those tables and then removed** (D-192, D-195). The engine's half of it was real -- meep has a `scope_filter` hook, fully wired, and component mutations go through it, so "no relevance filtering" was wrong about the engine -- and PVS culling measured 42.9 to **19.3 KB/s** per client on `oa_dm1` and nothing at all on `am_thornish`. It is gone because the trade is wrong for this genre: a player rounds a corner and shoots, so a visibility filter withholds an opponent until they are already firing, and every PVS boundary is where a fight starts. It also loses the *entity* rather than precision, where the other two levers lose only precision. **And bandwidth turns out not to be a problem at all** (D-196): sixteen players cost a client 245 KB/s down and 9.8 up on an 80 ms link, which is 1.96 Mbit/s and unremarkable in 2026. The 48 KB/s budget this step asserted against is undated and unexplained, so "5x over budget" was arithmetic against a modem-era number. What is left is a hosting cost line -- ~16 Mbit/s of host egress per eight humans, multiplied by concurrent matches -- and quantisation demoted from a bandwidth lever to a way of getting a tick under the MTU and out of the fragment path. Neither is urgent |
