...
| 7 — latency, loss, bandwidth | **done**. Both REPORT §5 tables written (D-183) and re-taken at 30 Hz (D-184): downstream **45.8 KB/s** per client, inside the 48 KB/s target; host CPU **29–65 ms/s**; event delivery now exact at every link. Superlinear host cost unchanged. **The feature both tables pointed at is built and measured** (D-192): meep had the `scope_filter` hook all along and component mutations go through it, and PVS culling takes `oa_dm1` from 42.9 to **19.3 KB/s** per client (55% of the bytes, a quarter of the packets) and `am_thornish` from 41.0 to 41.0 — so its value is a property of the map. Off by default, and **D-194 did not change why**: a player culled out of a client's PVS stops being published and freezes where they were, because the entity still exists locally and nothing says the state is stale. Reaping them client-side needs a way to *re*-introduce one when they come back into view, and the only re-introduction the engine offers is an INITIAL_SYNC -- whose `seek_to_frame` side effect makes it unusable at the rate a deathmatch crosses PVS boundaries. That is GAP-038's spawn action, and it is the thing standing between this measurement and shipping the feature |
