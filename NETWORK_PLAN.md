...
| 6 — full match for every slot | host half **done** (D-180); remote characters and missiles **done** and measured (D-181); client weapon selection **done** (D-182); **scoreboard HUD and remote footsteps done** (D-190), which found `bobCycle` publishing zero for every bot since step 3; **teleporters, jump pads and hurt volumes done on the host** (D-191) -- and GAP-041 was never blocking them, because a trigger is not solid. What is left: doors and plats non-solid on the host (GAP-041, narrowed), triggers unpredicted so a pad costs one correction, no transient *presentation* on the joined branch at all (so a teleport is silent), and bots use no trigger on either path. GAP-045 open upstream |
| 7 — latency, loss, bandwidth | **done**. Both REPORT §5 tables written (D-183) and re-taken at 30 Hz (D-184): downstream **45.8 KB/s** per client, inside the 48 KB/s target; host CPU **29–65 ms/s**; event delivery now exact at every link. Superlinear host cost unchanged. **The feature both tables pointed at is built and measured** (D-192): meep had the `scope_filter` hook all along and component mutations go through it, and PVS culling takes `oa_dm1` from 42.9 to **19.3 KB/s** per client (55% of the bytes, a quarter of the packets) and `am_thornish` from 41.0 to 41.0 — so its value is a property of the map. Off by default: a culled slot freezes rather than disappearing |
